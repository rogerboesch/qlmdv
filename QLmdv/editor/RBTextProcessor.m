...
@implementation RBTextProcessor
...
+ (RBTextProcessor*)defaultProcessor {
	return [[RBCProcessor new] autorelease];
}
...
+ (RBTextProcessor*)processorForExtension:(NSString*)extension {
    ...
    else if ([extension isEqualToString:@"asm"]) {
        return [[RBAsmProcessor new] autorelease];
    }
    else if ([[extension lowercaseString] isEqualToString:@"bas"]) {
        return [[RBSuperBasicProcessor new] autorelease];
    }
    else if ([[extension lowercaseString] isEqualToString:@"boot"]) {
        return [[RBSuperBasicProcessor new] autorelease];
    }
	else {
		return [[RBCProcessor new] autorelease];
...
- (BOOL)document:(TextDocument *)document textView:(NSTextView *)textView doCommandBySelector:(SEL)selector {
	if (selector == @selector(insertNewline:)) {
		// new line
		if ([RBTextSettings shared].autoIndentNewLines) {
			return NO;
		}
		...
		if (bracketRange.location == NSNotFound || (followingWhitespaceLength < [following length] && [following characterAtIndex:followingWhitespaceLength] == '}')) {
			[textView insertText:[NSString stringWithFormat:@"%@%@", [RBTextSettings shared].defaultLineEnding, [firstLine substringWithRange:NSMakeRange(0, whitespaceLength)]] replacementRange:selectedRange];
		}
		else {
			[textView insertText:[NSString stringWithFormat:@"%@%@%@", [RBTextSettings shared].defaultLineEnding, [firstLine substringWithRange:NSMakeRange(0, whitespaceLength)], [RBTextSettings shared].tabString] replacementRange:selectedRange];
		}
		...
		// shift left
		if (![self removePrefix:[RBTextSettings shared].tabString fromSelectedLinesInTextView:textView]) {
		...
		// shift right
		[self addPrefix:[RBTextSettings shared].tabString toSelectedLinesInTextView:textView];
...
- (BOOL)document:(TextDocument *)document textView:(NSTextView *)textView doKeyDownByEvent:(NSEvent *)event {
	if ([event keyCode] == 48 && !([NSEvent modifierFlags] & (NSEventModifierFlagControl | NSEventModifierFlagCommand | NSEventModifierFlagOption))) {
		// tab
		if ([event modifierFlags] & NSEventModifierFlagShift) {
			...
			if (selectedRange.length == 0) {
				// insert indent
				[textView insertText:[RBTextSettings shared].tabString];
			}
			else {
				// shift right
				[self addPrefix:[RBTextSettings shared].tabString toSelectedLinesInTextView:textView];
		...
		// closing bracket
		if (![RBTextSettings shared].autoIndentCloseBraces) {
			return NO;
		}
...
- (void)colorText:(NSColor*)color atRange:(NSRange)range textStorage:(NSTextStorage*)textStorage {
    if (range.location > _highlightNextHighlight) {
        ...
        [textStorage addAttribute:NSForegroundColorAttributeName value:[RBTextSettings shared].defaultColor range:r];
    }
...
- (void)uppercaseTextAtRange:(NSRange)range textStorage:(NSTextStorage*)textStorage {
    NSString* str = [[[textStorage attributedSubstringFromRange:range] string] uppercaseString];
